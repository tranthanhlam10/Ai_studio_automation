...
test.describe('Studio - Tài khoản hợp lệ', { tag: ['@ui', '@regression', '@authenticated'] }, () => {
  test.skip(!env.hasLoginCredentials,
  ...
  test('LOGIN-005: Đăng nhập thành công', async ({ loginPage, page }) => {
    ...
    await expect(page).toHaveURL(new URL(env.loginSuccessPath, env.baseURL).href);
